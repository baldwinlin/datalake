...
class Reformatter:
    ...
    @staticmethod
    def encoding_to_target_encoding(file_path: str, decoding: str, temp_path: str, target_encoding: str):
        with open(file_path, "rb") as f:
            content = f.read()

        # 使用strict模式，如果有解码错误就抛出异常
        try:
            decoded_content = content.decode(decoding, errors="strict")
        except UnicodeDecodeError as e:
            raise Exception(f"文件 {file_path} 包含無法用 {decoding} 解碼的內容: {e}")
